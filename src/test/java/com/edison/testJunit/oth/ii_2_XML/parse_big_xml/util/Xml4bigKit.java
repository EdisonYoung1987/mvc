...
public class Xml4bigKit {
		...
		try {
			if (len < MAX) {// small file
				...
				if (dataStartTag != null) {
					...
					// /Cars/car
					// 这里就传入"/Cars",表示解析/Cars下面的/car,而不是/car或者/Cars/oth/car
				}
				...
				for (Class<? extends XmlBase> cl : cls) {
					...
					if (listElement != null) {
						...
						for (Element element : listElement) {
							...
							Method proccessMethod = obj.getClass().getMethod(
									proccessMethodName, Element.class);
				...
				while ((rlen = in.read(cbuf)) != -1) {
					/**
					 * 需考虑几种情况： 1.这个标签内容多，READ_LEN的内容装不下 2.这个标签内容少，READ_LEN里面有多个
					 */
					if (rlen != READ_LEN) {
					...
					parseXmlByStr(sbcache, tagsList, reMap, cls, handle,
							tagMaxLen, xmlfilter);

					count++;
					if (count % 1000 == 0) {
						System.out.println(String.format("当前进度:%5.2f %%" , 100.00f*count*READ_LEN/file.length()));
					}
				}
	 ...
	 *            待解析xml文件路径
	 * @param dataStartTag
	 *            起始标签: null-从根标签开始解析 "/root/abc"-从
	 *            &lt;root&gt;&lt;abc&gt;标签开始解析
	 * @param cls
	 *            待保存解析对象的类的list，暂时不支持同时解析多个标签内容到对象
	 * @param handle
	 *            传递解析好的List&lt;Object&gt;到外部调用对象
	 * @param xmlfilter
	 *            通过xml的Element进行过滤
	 * @throws Exception
	 */
	public static void parseXml(String filepath, String dataStartTag,
			List<Class<? extends XmlBase>> cls, XmlHandler handle,
			XmlFilter xmlfilter) throws Exception {
		if (cls.size() == 0) {
	 ...
	 */
	public static void parseXmlByStr(StringBuffer sbcache,
			List<String> tagsList, Map<String, List<Object>> reMap,
			List<Class<? extends XmlBase>> cls, XmlHandler handle,
			int tagMaxLen, XmlFilter xmlfilter) throws InstantiationException,
		...
		if (range.getFrom() >= 0) {
			...
			if (range.getTo() > 0) {// 说明找到了一组对应标签
				elementText = sbcache.substring(range.getFrom(),
						range.getTo() + 1);
				Document doc;
				doc = DocumentHelper.parseText("<Root>" + elementText
						+ "</Root>");
